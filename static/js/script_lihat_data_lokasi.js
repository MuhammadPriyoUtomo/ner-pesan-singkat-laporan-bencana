document.addEventListener('DOMContentLoaded', function () {
    ...
    const lastPage = localStorage.getItem('lastPageLihatDataLokasi');
    const hasRedirected = localStorage.getItem('hasRedirectedLihatDataLokasi');

    // 1. Jika tidak ada data dan bukan halaman default: kembali ke halaman default
    if (noDataFound && currentUrl === '/lihat_lokasi' && currentPage !== "1") {
        localStorage.removeItem('lastPageLihatDataLokasi');
        window.location.href = '/lihat_lokasi';
        return;
    }

    // 2. Jika tidak ada data dan sudah di halaman default: jangan apa-apa
    if (noDataFound && currentUrl === '/lihat_lokasi') {
        return;
    }

    // 3. Jika ada data tapi belum di halaman terakhir, redirect
    if (lastPage && !hasRedirected && parseInt(currentPage) !== parseInt(lastPage)) {
        localStorage.setItem('hasRedirectedLihatDataLokasi', 'true');
        ...
        localStorage.removeItem('hasRedirectedLihatDataLokasi');
    }

    // 4. Simpan halaman saat klik pagination
    document.querySelectorAll('.pagination .page-link').forEach(link => {
        link.addEventListener('click', function () {
            const page = this.dataset.page || new URL(this.href).searchParams.get('page');
            if (page) {
                localStorage.setItem('lastPageLihatDataLokasi', page);
            }
        });
    });
