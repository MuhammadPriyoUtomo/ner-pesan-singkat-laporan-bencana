document.addEventListener('DOMContentLoaded', function () {
    ...
    const lastPage = localStorage.getItem('lastPageResultsSimulasi');
    const hasRedirected = localStorage.getItem('hasRedirectedResultsSimulasi');

    // 1. Jika tidak ada data dan bukan halaman default: kembali ke halaman default
    if (noDataFound && currentUrl === '/results_hasil_simulasi' && currentPage !== "1") {
        localStorage.removeItem('lastPageResultsSimulasi');
        window.location.href = '/results_hasil_simulasi';
        return;
    }

    // 2. Jika tidak ada data dan sudah di halaman default: jangan apa-apa
    if (noDataFound && currentUrl === '/results_hasil_simulasi') {
        return;
    }

    // 3. Jika ada data tapi belum di halaman terakhir, redirect
    if (lastPage && !hasRedirected && parseInt(currentPage) !== parseInt(lastPage)) {
        localStorage.setItem('hasRedirectedResultsSimulasi', 'true');
        ...
        localStorage.removeItem('hasRedirectedResultsSimulasi');
    }
    ...
    document.querySelectorAll('.page-item form').forEach(form => {
        form.addEventListener('submit', function (e) {
            ...
            if (page) {
                localStorage.setItem('lastPageResultsSimulasi', page);
                const url = new URL(window.location.href);
